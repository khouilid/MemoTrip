...
def generate_shared_offline_files(parent_dir):
...
import '../infrastructure/local/{file_name_prefix}_local_datasource.dart';
...
final {file_name_prefix.replace("_", "").lower()}RepositoryProvider = Provider<{file_name_prefix.capitalize().replace("_", "")}Repository>(
  (ref) => {file_name_prefix.capitalize().replace("_", "")}Repository(
    ...
    ref.watch({file_name_prefix.replace("_", "").lower()}LocalRepositoryProvider),
